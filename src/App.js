      ...
      <main >
        <div style={lang === "ar" ? {display: "flex", flexDirection: "row-reverse"} :{ display: "flex"}} className="flexing-index-intro">
          <div className='index-intro-content'>
            ...
            <button className="start-button-index" id={lang === "ar" && "ar-start-btn"} style={lang === "ar"?{marginLeft: "auto",display: "flex", marginRight: "3rem"} : {}} onClick={() => isAccess ? window.location.replace("https://localhost:3000/home") : window.open('http://codexbot.xyz/login')}>{t("indexPage.indexGetstarted")}</button>
          </div>
